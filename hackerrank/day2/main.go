...
func solve(meal_cost float64, tip_percent int32, tax_percent int32) {
	...
	tip_cal := (meal_cost * float64(tip_percent)) / 100
	tax_cal := (meal_cost * float64(tax_percent)) / 100
	total_mealcost := meal_cost + tip_cal + tax_cal
	fmt.Println(math.Round(total_mealcost))
